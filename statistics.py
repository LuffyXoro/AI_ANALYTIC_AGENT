...
def compare_detectors(
    ...
    """
    Run all detectors against a metric/region(/category) slice with a KNOWN
    anomaly window, and report recall (did each method flag the known
    anomaly?) and false positives (did it flag days OUTSIDE the window?).

    This is what turns "we built several detectors" into "here's actual
    evidence for which one is better suited to which failure pattern" --
    per the brief's requirement not to blindly choose a method.
    """
    sub = df
